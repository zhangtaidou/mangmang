        ...
        $.ajax({
            ...
            success:function (data){
                ...
                $.each(data, function (index, poem) {
                    ...
                    name.attr('style', "width: 11%")
                    //商品图片
                    var img_src = $("<img style=height:30px;width:40px; src="+poem.img_src+">");
                    var td=$("<td>");
                    td.append(img_src)
                    td.attr('style', "width: 11%")

                    var price = $("<td>").html(poem.price+"￥")
                    price.attr('style', "width: 11%")
                    var stock = $("<td>").html(poem.stock)
                    stock.attr('style', "width: 11%")
                    var sales = $("<td>").html(poem.sales)
                    sales.attr('style', "width: 11%")
                    var date = $("<td>").html(poem.date)
                    date.attr('style', "width: 11%")
                    var kind = $("<td>").html(poem.kind)
                    kind.attr('style', "width: 11%")

                    var operate = $("<br> <input type='button' id='input' onclick='del ("+poem.id+")' value='删除' class='btn btn-danger'>")
                    operate.attr('style', "width: 20%")
                    var tr = $("<tr>").append(name).append(td).append(price).append(stock).append(sales).append(date).append(kind).append(operate)
                    $("#tr").after(tr)
                })
            },
...
<table id="table" table width="100%" border="2"cellpadding="2" cellspacing="1" >
    <tr id="tr">
        ...
        <th style="width: 11%">商品图片</th>
        <th style="width: 11%">商品价格</th>
        <th style="width: 11%">商品库存</th>
        <th style="width: 11%">商品销量</th>
        <th style="width: 11%">上架时间</th>
        <th style="width: 11%">商品分类</th>
